...
export class StockProvider {
  ...
  getOneStokcs(idProducto:string):Observable<StockItem[]>{
    return this.db.list(`${SUC_STOCK_ROOT}${idProducto}/Stocks/`).map((snap:StockItem[])=>{
  ...
  setMpp(stock: Stock): Observable<string> {
    return new Observable((obs) => {
      this.db.object(`${SUC_STOCK_ROOT}${stock.id}/Stocks/`).set(stock.Stocks).then(() => {
